...
def print_distribution_comparison(y_true, pred_proba):
    ...
    for i, outcome in enumerate(OUTCOME_ORDER):
        print(f"  {outcome:<6} {true_freqs[i]*100:>7.2f}% {pred_freqs[i]*100:>9.2f}% "
...
def print_feature_importance(model, feature_names, top_n=15):
    ...
    for i, row in fi.head(top_n).iterrows():
        print(f"  {row['feature']:<20} gain={row['gain']:>12,.0f} "
...
def diagnostic_model_discrimination(
    ...
    for label, idx in examples:
        ...
        print(f"\n  {label}:")
        print(f"    b_k_rate={row['b_k_rate']:.3f}  b_iso={row['b_iso']:.3f}  "
              f"b_hr_rate={row['b_hr_rate']:.3f}")
        print(f"    p_k_rate={row['p_k_rate']:.3f}  p_avg={row['p_avg']:.3f}  "
    ...
    scenarios = {
        "Elite batter vs Elite pitcher": {
            "b_k_rate": 0.12, "b_bb_rate": 0.13, "b_avg": 0.310,
            "b_obp": 0.420, "b_slg": 0.580, "b_iso": 0.270, "b_hr_rate": 0.060,
            "p_k_rate": 0.32, "p_bb_rate": 0.05, "p_avg": 0.200,
            "p_obp": 0.260, "p_slg": 0.320, "p_iso": 0.120, "p_hr_rate": 0.018,
        },
        "Elite batter vs Bad pitcher": {
            "b_k_rate": 0.12, "b_bb_rate": 0.13, "b_avg": 0.310,
            "b_obp": 0.420, "b_slg": 0.580, "b_iso": 0.270, "b_hr_rate": 0.060,
            "p_k_rate": 0.16, "p_bb_rate": 0.11, "p_avg": 0.280,
            "p_obp": 0.360, "p_slg": 0.490, "p_iso": 0.210, "p_hr_rate": 0.045,
        },
        "Bad batter vs Elite pitcher": {
            "b_k_rate": 0.32, "b_bb_rate": 0.05, "b_avg": 0.200,
            "b_obp": 0.260, "b_slg": 0.320, "b_iso": 0.120, "b_hr_rate": 0.012,
            "p_k_rate": 0.32, "p_bb_rate": 0.05, "p_avg": 0.200,
            "p_obp": 0.260, "p_slg": 0.320, "p_iso": 0.120, "p_hr_rate": 0.018,
        },
        "Average batter vs Average pitcher": {
            "b_k_rate": 0.226, "b_bb_rate": 0.082, "b_avg": 0.243,
            "b_obp": 0.312, "b_slg": 0.399, "b_iso": 0.156, "b_hr_rate": 0.029,
            "p_k_rate": 0.226, "p_bb_rate": 0.082, "p_avg": 0.243,
            "p_obp": 0.312, "p_slg": 0.399, "p_iso": 0.156, "p_hr_rate": 0.029,
        },
    }
    ...
    for label, overrides in scenarios.items():
        ...
        probs = model.predict(features.reshape(1, -1),
    ...
    for label in scenarios:
        short = label.replace(" vs ", " v ").replace("Elite", "Elt").replace("Average", "Avg").replace("Bad", "Bad")
...
if __name__ == "__main__":
    ...
    print(f"  {'liga avg':<10} log_loss={ll_league:.4f}  (predecir frecuencia de train siempre)")
